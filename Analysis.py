...
load_data = json.loads(sys.argv[2]) # load_data --> identity || type || ph1 || ph1 || ...
loaddata = pd.DataFrame(load_data)

config_data = json.loads(sys.argv[3]) # config data --> identity || 9 R and X values
configdata = pd.DataFrame(config_data)

trans_data = json.loads(sys.argv[4]) # trans data --> name || Power || high kv || low kv || R || X
transdata = pd.dataFrame(trans_data)

dist_data = json.loads(sys.argv[5]) # dist data--> Node A || node B || type || ph1 || ph 1 ....
distdata = pd.dataFrame(dist_data)

shunt_data = json.loads(sys.argv[6]) # shunt data --> Node || ph-1 || ph-2 || ph-3
